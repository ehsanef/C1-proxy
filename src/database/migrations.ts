/**
 * Idempotent migration engine for Cloudflare D1.
 * Allows C1 Proxy to run migrations automatically on startup without requiring manual CLI commands.
 */
...
/**
 * Ensures all schema migrations are applied idempotently.
 * Caches migration state in-memory so warm requests do not query sqlite_master repeatedly.
 */
export async function ensureMigrations(db: D1Database, force = false): Promise<void> {
  ...
  try {
    ...
    const result = await db.prepare('SELECT version FROM schema_migrations ORDER BY version ASC;').all<{ version: number }>();
    const appliedVersions = new Set<number>((result.results || []).map((r) => r.version));
